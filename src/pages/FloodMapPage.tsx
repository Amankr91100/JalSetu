  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      shelters.forEach((s) => {
        ...
        el.className =
          "w-3 h-3 rounded-full bg-success-500 border-2 border-white shadow";
        el.title = s.name;
      ...
      <div className="absolute top-3 left-3 right-3 sm:left-4 sm:right-auto flex flex-col sm:flex-row gap-2 z-10 max-w-md">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() =>
              mapRef.current?.flyTo({ center: [91.7, 26.2], zoom: 10 })
            }
          >
      ...
      <div className="absolute bottom-24 sm:bottom-4 left-3 z-10 glass-card rounded-lg p-2.5 text-xs shadow-card">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded bg-flood-500/60 border border-flood-600" />{" "}
            Flood / water
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded bg-warning-400/60 border border-warning-600" />{" "}
            Moderate risk
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded bg-orange-500/60 border border-orange-600" />{" "}
            High risk
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded bg-danger-500/60 border border-danger-600" />{" "}
            Critical
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-success-500 border-2 border-white" />{" "}
            Shelter
          </div>
        </div>
      </div>
        ...
        <div className="lg:hidden absolute bottom-0 left-0 right-0 z-20 bg-white border-t border-navy-200 rounded-t-2xl p-4 shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-2">
            {LAYER_DEFS.map((l) => (
              <label
                key={l.id}
                className="flex items-center gap-2 text-sm p-2 rounded-lg hover:bg-navy-50"
              >
                <input
                  type="checkbox"
                  checked={layers[l.id]}
          ...
          <div className="p-4 space-y-3 text-sm">
            ...
            <div className="flex gap-4">
              <div>
                ...
                <p className="font-medium text-flood-700">
                  {selectedInfo.status}
                </p>
              </div>
              <div>
            ...
            <div className="flex gap-4 text-xs">
              <span>
                Nearby shelters: <strong>{selectedInfo.sheltersNearby}</strong>
              </span>
              <span>
                Hospitals: <strong>{selectedInfo.hospitalsNearby}</strong>
              </span>
            </div>
